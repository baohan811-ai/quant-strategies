...
STOP_DRAWDOWN = 0.1
LOOKBACK_DAYS = 1600
TRADE_START_DATE = "2023-04-07"  # 必须写成字符串，例如 "2025-01-01"；None 表示沿用当前逻辑
MAX_TURNOVER = 0.40
VOLATILITY_SPIKE_LIMIT = 1.5
MIN_AVG_AMOUNT_20D = 10_000_000
LOW_AMOUNT_PENALTY_WEIGHT = 0.5
...
print("全部港股股票数量：", len(stock_codes))
...
def get_cache_path(field):
    return os.path.join(CACHE_DIR, f"全部港股_{field}_PriceAdjF.pkl")
...
turn_df = get_price_df_with_cache(stock_codes, "turn")
amt_df = get_price_df_with_cache(stock_codes, "amt")

valid_trading_mask = turn_df.notna().sum(axis=1) > 0
open_df = open_df.loc[valid_trading_mask]
high_df = high_df.loc[valid_trading_mask]
close_df = close_df.loc[valid_trading_mask]
turn_df = turn_df.loc[valid_trading_mask]
amt_df = amt_df.loc[valid_trading_mask]
...
turn_ma20 = turn_df.rolling(20).mean()
amt_ma20 = amt_df.rolling(20).mean()
volatility_20 = daily_ret.rolling(20).std()
volatility_60 = daily_ret.rolling(60).std()
turnover_ratio = turn_df / turn_ma20.replace(0, np.nan)
turnover_score = (turnover_ratio - 1).clip(lower=0, upper=1).fillna(0)
overheat_penalty = ((turn_df / MAX_TURNOVER) - 1).clip(lower=0).fillna(0)
low_amount_penalty = (
    (MIN_AVG_AMOUNT_20D / amt_ma20.replace(0, np.nan)) - 1
).clip(lower=0, upper=2).fillna(2)
volatility_ratio = volatility_20 / volatility_60.replace(0, np.nan)
volatility_score = (
    (VOLATILITY_SPIKE_LIMIT - volatility_ratio).clip(lower=0, upper=VOLATILITY_SPIKE_LIMIT)
    / VOLATILITY_SPIKE_LIMIT
).fillna(0)
candidate_score = (
    (ma5 / ma60 - 1)
    + 0.5 * (ma60 / ma60.shift(5) - 1)
    + 0.2 * turnover_score
    + 0.2 * volatility_score
    - 0.2 * overheat_penalty
    - LOW_AMOUNT_PENALTY_WEIGHT * low_amount_penalty
)
...
output_file = os.path.join(output_dir, f"金叉买入_分层回撤卖出策略_全部港股_{end_date}.xlsx")
